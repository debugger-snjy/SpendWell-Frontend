...
function NavBreadcrumb() {
    ...
    let userrole = sessionStorage.getItem("role").toLowerCase();
    ...
    const breadcrumbItems = pathSegments.map((segment, index) => {
        ...
        if (path.endsWith("category")) {
            ...
            path = `${path}/${userrole}`
            console.log("NAVBAR 2 : ", path)
        }
        else if (path.endsWith(`${userrole}`)) {
            path = path.split("/")[0]
            console.log("NAVBAR : Path here : ", path)
        }
        ...
        if(name.toLowerCase() === "mstatement"){
            name = "Mini Statement"
        }
    ...
    return (
        <nav style={customStyle} aria-label="breadcrumb" className='menutextStyle my-3'>
